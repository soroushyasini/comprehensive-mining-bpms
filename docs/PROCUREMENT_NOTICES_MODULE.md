...
دکمه «خروجی اکسل» دقیقاً از آخرین پرس‌وجوی موفقی استفاده می‌کند که جدول روی صفحه را ساخته است. پنل همهٔ صفحات آن نتیجه را از action موجود `list` و در دسته‌های ۱۰۰تایی دریافت می‌کند؛ بنابراین خروجی به صفحهٔ فعلی محدود نیست. سپس کتابخانه same-origin نصب‌شده در ProcessMaker از مسیر `/lib/xlsx.full.min_2.js` یک فایل واقعی `.xlsx` با برگهٔ راست‌به‌چپ، سرستون، فیلتر خودکار و تمام فیلدهای قابل گزارش می‌سازد. مقدارهای ۲۴رقمی مبلغ برآوردشده به‌صورت متن قالب‌بندی‌شده صادر می‌شوند تا Excel دقت آن‌ها را کم نکند.
