...
دکمه «خروجی اکسل» دقیقاً از آخرین پرس‌وجوی موفقی استفاده می‌کند که جدول روی صفحه را ساخته است. پنل همهٔ صفحات آن نتیجه را از action موجود `list` و در دسته‌های ۱۰۰تایی دریافت می‌کند؛ بنابراین خروجی به صفحهٔ فعلی محدود نیست. سپس کتابخانه same-origin نصب‌شده در ProcessMaker از مسیر `/lib/xlsx.full.min_2.js` یک فایل واقعی `.xlsx` با برگهٔ راست‌به‌چپ، سرستون، فیلتر خودکار و تمام فیلدهای قابل گزارش می‌سازد. لایه سازگاری پنل API جدید `aoa_to_sheet` و API قدیمی `sheet_from_array_of_arrays` را می‌پذیرد؛ اگر هیچ‌یک موجود نباشند، پنل Worksheet و آدرس سلول‌ها را بدون وابستگی به `XLSX.utils` می‌سازد و فقط از writer کتابخانه استفاده می‌کند. writerهای `writeFile` و `write` هر دو پشتیبانی می‌شوند، اما در مرورگر مسیر `write` و دانلود `Blob` عمداً اولویت دارد؛ زیرا بعضی buildهای قدیمی `writeFile` را اشتباهاً به API مخصوص Node یعنی `writeFileSync` هدایت می‌کنند. مقدارهای ۲۴رقمی مبلغ برآوردشده به‌صورت متن قالب‌بندی‌شده صادر می‌شوند تا Excel دقت آن‌ها را کم نکند.
