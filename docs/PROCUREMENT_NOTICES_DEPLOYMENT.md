...
30. فایل `/lib/xlsx.full.min_2.js` با وضعیت 200 بارگیری شود؛ پنل باید API جدید `aoa_to_sheet` یا API قدیمی `sheet_from_array_of_arrays` را تشخیص دهد و هنگام ساخت خروجی، دکمه غیرفعال و پیشرفت جمع‌آوری رکوردها به فارسی نمایش داده شود.
